      ...
      <div className='right flex items-center justify-center w-full min-h-[45vh] pt-4 md:h-[90vh] md:w-3/5' style={{ background: background.secondary }}>
        ...
        <div className='flex flex-col gap-1 md:gap-2 pt-3' style={{ color: background.textprimary }}>
          ...
          <div className="flex items-center justify-center md:justify-start mt-3 md:mt-6 gap-6">
            {icons.map(item => (
              <motion.div whileHover={{ y: -3 }} whileTap={{ scale: 0.9 }} key={item.link} className='p-2 cursor-pointer flex items-center justify-center rounded-full' style={{ color: 'white', borderColor: color, background: color }}>
                <a href={item.link} target="_blank" >{item.icon}</a>
              </motion.div>
            ))}
          </div>
          <div className='py-4'>
            <AnimatedButton link="/articles" text="View Articles"/>
          </div>
        </div>
